...
test("chart cards use an icon-only inline expand action", () => {
  const app = read("../src/analytics-app/App.tsx");
...
test("artifact top bar matches the title-date-overflow-edit layout", () => {
  ...
  const freshness = app
    ...
    .split("function AnalyticsTopBar(", 1)[0];
  const topBar = app
  ...
  assert.match(freshness, /const refreshLabel = dateLabel === "Unknown" \? "Refresh" : dateLabel;/);
  assert.match(freshness, /aria-label=\{`Refresh \$\{surfaceLabel\}`\}/);
  assert.match(freshness, /className="top-bar-refresh-datetime"/);
  assert.match(freshness, /onFocus=\{positionRefreshTooltip\} onMouseEnter=\{positionRefreshTooltip\}/);
  assert.match(freshness, /const positionRefreshTooltip = useCallback\(\(\) => \{/);
  assert.match(freshness, /const tooltipWidth = Math\.min\(320, Math\.max\(0, window\.innerWidth - \(viewportPadding \* 2\)\)\);/);
  assert.match(freshness, /setTooltipLeft\(Math\.min\(Math\.max\(buttonRect\.left, viewportPadding\), maxLeft\)\);/);
  assert.match(freshness, /window\.addEventListener\("resize", positionRefreshTooltip\);/);
  assert.match(freshness, /window\.removeEventListener\("resize", positionRefreshTooltip\);/);
  assert.match(freshness, /<span className="top-bar-refresh-text">\{refreshLabel\}<\/span>/);
  assert.match(freshness, /Last refreshed \$\{dateLabel\}\. Click to ask Codex to refresh this \$\{surfaceLabel\}\./);
  assert.match(freshness, /className="top-bar-refresh-tooltip"[^>]*role="tooltip"/);
  assert.doesNotMatch(freshness, /RefreshCw|top-bar-button/);
  assert.ok(topBar.indexOf('className="analytics-top-bar-leading"') < topBar.indexOf('className="analytics-top-bar-actions"'));
  assert.ok(topBar.indexOf("<AnalyticsTopBarFreshness") < topBar.indexOf('className="analytics-top-bar-actions"'));
  assert.ok(topBar.indexOf('className="export-menu"') < topBar.indexOf('className="top-bar-button top-bar-edit-button"'));
  assert.match(topBar, /aria-label=\{`More \$\{appSurfaceLabel\(manifest\)\} actions`\}/);
  assert.match(topBar, /<Ellipsis aria-hidden="true" size=\{18\} strokeWidth=\{2\}\/>/);
  assert.match(topBar, /<Pencil aria-hidden="true" size=\{16\} strokeWidth=\{2\}\/>/);
  assert.doesNotMatch(topBar, /<span>Export<\/span>|<ChevronDown/);
  assert.match(topBar, /exportTargets\.map\(\(target\) => \(<button className="export-menu-item"/);
  assert.match(app, /void submitCodexPrompt\([\s\S]*refreshPrompt\(manifest, snapshot, packageInfo\),[\s\S]*`Refresh \$\{appSurfaceLabel\(manifest\)\}`/);
  assert.ok(submitPrompt);
  assert.match(submitPrompt, /await sendCodexPromptToHost\(prompt, title\)/);
  assert.match(submitPrompt, /isPublishedArtifactSite\(packageInfo\)[\s\S]*copyTextToClipboard\(prompt\)/);
  ...
  assert.match(app, /const result = await hostApi\.sendFollowUpMessage\(codexHostPromptPayload\(prompt, title\)\);[\s\S]*result\?\.isError !== true/);
  assert.match(app, /const result = await hostApi\.sendMessage\(codexMessagePayload\(prompt\)\);[\s\S]*result\?\.isError !== true/);
  assert.doesNotMatch(app, /hostApi\?\.openCodexPrompt/);
...
test("artifact fallback uses encoded chart fields and fullscreen language", () => {
  ...
  assert.match(source, /y: \{ field: "revenue", type: "quantitative", label: "Revenue"/);
  assert.match(source, /cardIds: \["net_revenue_card", "target_attainment_card", "refund_rate_card", "completed_orders_card"\]/);
  assert.match(source, /tableId: "revenue_summary_table"/);
  assert.match(source, /tableId: "revenue_detail_table"/);
