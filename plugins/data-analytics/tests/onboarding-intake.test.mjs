...
test("guided intake uses two connector tasks plus upload your own or upload and sample fallbacks", () => {
  ...
  assert.match(intake, /On Codex desktop, call `request_user_input` whenever it is available/);
  assert.match(intake, /On every other surface or mode, invoke `\$answers-ask-user-input` whenever it is available/);
  assert.match(intake, /invoke `request_user_input` on Codex desktop or `\$answers-ask-user-input` elsewhere once with exactly three options/);
